...
internal func printNetwork(_ nodes: [KadDHT.Node]) {
    ...
    print(
        "Nodes that are left of all their peers: \(farLeftNodes.map { $0.peerID.description }.joined(separator: ", "))"
    )
    ...
    print(
        "Nodes that are right of all their peers: \(farRightNodes.map { $0.peerID.description }.joined(separator: ", "))"
    )
...
func normalizedCurrentOrdering(_ nodes: [KadDHT.Node]) -> [KadDHT.Node] {
    nodes.sorted { lhs, rhs in
        ...
        let lhsSummation = try! lhs.routingTable.getPeerInfos().wait().sortedAbsolutely().prefix(maxCommonPeers).reduce(
            [UInt8](),
            { partialResult, peer in
                sumByteArrays(partialResult, bytes2: peer.id.absoluteDistance())
            }
        )
        let rhsSummation = try! rhs.routingTable.getPeerInfos().wait().sortedAbsolutely().prefix(maxCommonPeers).reduce(
            [UInt8](),
            { partialResult, peer in
                sumByteArrays(partialResult, bytes2: peer.id.absoluteDistance())
            }
        ...
        if lhsSummation.count < rhsSummation.count {
            ...
            return KadDHT.Key(preHashedBytes: [UInt8](repeating: 0, count: lhsSummation.count))
                .compareDistancesFromSelf(
                    to: KadDHT.Key(preHashedBytes: lhsSummation),
                    and: KadDHT.Key(preHashedBytes: rhsSummation)
                ) == .firstKey
        }
    }.reversed()
}
